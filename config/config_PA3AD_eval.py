...
def set_deterministic_config(seed=3407):
    random.seed(seed)
    np.random.seed(seed)
...
def get_parser():
    parser = argparse.ArgumentParser(description='PA3AD 3D Anomaly Detection Evaluation')
    parser.add_argument('--config', type=str, default='config/config_PA3AD_eval.py', help='config file')
    ...
    parser.add_argument('--task', type=str, default='eval', help='task type: eval')

    # Dataset
    parser.add_argument('--dataset', type=str, default='AnomalyShapeNet',
                       choices=['AnomalyShapeNet', 'Real3D'], help='dataset name')
    parser.add_argument('--category', type=str, default='vase7', 
                       help='category name (AnomalyShapeNet: vase0, bottle, car... / Real3D: airplane, candybar, car...)')

    parser.add_argument('--batch_size', type=int, default=1, help='batch size for evaluation (usually 1)')
    ...
    # Contrastive params (not used during eval, but required by dataset class)
    ...
    # Optimizer (not used during eval)
    ...
    # Training params (not used during eval)
    parser.add_argument('--epochs', type=int, default=50, help='number of epochs')
    parser.add_argument('--step_epoch', type=int, default=20, help='learning rate decay step')
    parser.add_argument('--save_freq', type=int, default=5, help='save frequency')

    # Evaluation
    parser.add_argument('--logpath', type=str, default='./log/vase7_transformer_fixed/', help='log path')
    parser.add_argument('--checkpoint_name', type=str, default='best_model.pth', help='checkpoint filename')

    # Data saving
    parser.add_argument('--save_input_data', action='store_true', help='save input data to file')
    parser.add_argument('--data_save_dir', type=str, default=None, help='data save directory (default: logpath/saved_input_data/)')
    parser.add_argument('--save_data_format', type=str, default='both', choices=['pickle', 'numpy', 'both'], 
                       help='data save format: pickle(full), numpy(arrays), both')
    ...
    # Auto-adjust params based on dataset
    if args.dataset == 'Real3D':
        if args.mask_num == 50:
            args.mask_num = 64

    # Create eval log directory
    eval_time = time.strftime('%Y%m%d_%H%M%S', time.localtime())
    args.eval_logpath = os.path.join(args.logpath, f'eval_{eval_time}/')
    os.makedirs(args.eval_logpath, exist_ok=True)

    # Ensure logpath ends with '/'
    if not args.logpath.endswith('/'):
        args.logpath += '/'

    print(f"Eval config summary:")
