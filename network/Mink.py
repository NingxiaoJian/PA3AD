...
class MinkUNetBase(ResNetBase):
    ...
    def forward(self, x): #torch.Size([194617, 3])

        out = self.conv0p1s1(x) #torch.Size([194617, 32])
        out = self.bn0(out) #torch.Size([194617, 32])
        out_p1 = self.relu(out)

        out = self.conv1p1s2(out_p1) #torch.Size([43498, 32]
        out = self.bn1(out)
        out = self.relu(out)
        out_b1p2 = self.block1(out)  #torch.Size([43498, 32])

        out = self.conv2p2s2(out_b1p2) #torch.Size([10515, 32])
        out = self.bn2(out)
        out = self.relu(out)
...
class MinkUNet34C_concet(MinkUNet34):
    PLANES = (32, 64, 128, 256, 256, 128, 96, 96)
...
def Mink_unet(in_channels=3, out_channels=20, D=3, arch='MinkUNet18A'):
    ...
    elif arch == 'MinkUNet34C_concet':
        return MinkUNet34C_concet(in_channels, out_channels, D)
    ...
    elif arch.startswith('MinkUNet') and 'Transformer' in arch:
        from network.MinkUNet_Transformer import MinkUNet_Transformer
        return MinkUNet_Transformer(in_channels, out_channels, D, arch)
    else:
